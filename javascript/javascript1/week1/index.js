...
function dogAgify() {
  ...
  var dogOrHuman = window.prompt(
    "DO you like to calculate in dog year or human? (d or h) "
  );
...
function housePricing() {
  ...
  var gardenSizeInM2 = window.prompt(
    "What the size of garden in square meter?"
  );
...
function startUpName() {
  ...
  var startUpName =
    firstWords[Math.floor(Math.random() * firstWords.length + 0)] +
    " " +
    secondWOrds[Math.floor(Math.random() * firstWords.length + 0)];
  return startUpName;
}
